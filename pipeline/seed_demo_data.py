...
try:
    from config import supabase_request, supabase_rest, log
...
REQUIRED_ROLES = ("student", "investor", "founder", "citizen")


def slugify_title(title: str) -> str:
    slug = re.sub(r"[^a-z0-9]+", "-", title.lower()).strip("-")
    return slug[:80] or "demo-article"


def infer_entities(article_data: dict[str, Any]) -> list[str]:
    if article_data.get("entities") and isinstance(article_data["entities"], list):
        return article_data["entities"][:8]

    title = article_data.get("title", "")
    tokens = re.findall(r"[A-Za-z][A-Za-z0-9&.-]+", title)
    entities = [t for t in tokens if len(t) > 2 and not t.islower()]
    if not entities:
        entities = [article_data.get("source", "India"), article_data.get("category", "Markets")]
    return entities[:8]


def build_seed_insert_data(article_data: dict[str, Any]) -> dict[str, Any]:
    title = article_data["title"]
    url_slug = slugify_title(title)
    category = article_data.get("category", "Global")

    role_headlines = {
        "student": f"Career lens: {title[:58]}",
        "investor": f"Market signal: {title[:58]}",
        "founder": f"Build window: {title[:58]}",
        "citizen": f"Household impact: {title[:58]}",
    }
    ...
    return {
        ...
        "source_url": f"https://etpatrika.demo/source/{url_slug}",
        "category": category,
        "story_arc_key": article_data.get("story_arc_key"),
        "published_at": article_data.get("published_at"),
        "content": article_data["content"],
        "url": f"https://etpatrika.demo/{url_slug}",
        "entities": infer_entities(article_data),
        "sentiment": article_data.get("sentiment", "neutral"),
        "credibility_score": article_data.get("credibility_score", 0.82),
        "story_momentum": article_data.get("story_momentum", "building"),
        "bharat_india_split": article_data.get("bharat_india_split", "both"),
        "conflict_index": article_data.get("conflict_index", 0.6),
        "role_headlines": role_headlines,
        "key_concepts": article_data.get("key_concepts", [category.lower(), "india", "policy"]),
    }


def _default_action_cards(role: str, title: str) -> list[dict[str, str]]:
    if role == "student":
        return [
            {"title": "Skill Signal", "description": "Identify one skill trend from this story.", "cta": "Add to learning plan"},
            {"title": "Placement Impact", "description": "Track which sectors may hire more freshers.", "cta": "Bookmark hiring sectors"},
        ]
    if role == "investor":
        return [
            {"title": "Portfolio Check", "description": "Review exposure to affected sectors.", "cta": "Rebalance watchlist"},
            {"title": "Catalyst Tracker", "description": "Track next trigger event for this theme.", "cta": "Set price alerts"},
        ]
    if role == "founder":
        return [
            {"title": "Product Opportunity", "description": "Map where this opens a build wedge.", "cta": "Draft GTM test"},
            {"title": "Regulatory Risk", "description": "List policy constraints for your model.", "cta": "Prepare compliance checklist"},
        ]
    return [
        {"title": "Household Impact", "description": "Estimate monthly wallet impact for your family.", "cta": "Plan monthly budget"},
        {"title": "What To Verify", "description": "Track official updates from trusted sources.", "cta": "Follow verified alerts"},
    ]


def apply_fallback_article_fields(article_id: str, article_data: dict[str, Any]) -> None:
    title = article_data.get("title", "This story")
    content = (article_data.get("content") or "").strip()
    eli5 = f"Simple baat hai: {title}. Toh iska matlab? Yeh update directly aapke decisions ko affect kar sakta hai."
    synthesis = (
        f"{title}\n\n"
        f"Core update: {content[:420]}\n\n"
        f"Market + policy implications are active across stakeholders.\n\n"
        f"Watch the next official update cycle for confirmation."
    )

    update_data = {
        "eli5": eli5,
        "synthesis_briefing": synthesis,
        "story_momentum": "building",
        "bharat_india_split": "both",
        "conflict_index": 0.62,
        "key_concepts": article_data.get("key_concepts", ["india", "business", "policy"]),
        "role_headlines": {
            "student": f"Career angle: {title[:58]}",
            "investor": f"Market angle: {title[:58]}",
            "founder": f"Build angle: {title[:58]}",
            "citizen": f"Family angle: {title[:58]}",
        },
        "credibility_score": article_data.get("credibility_score", 0.8),
    }
    supabase_request("PATCH", "articles", data=update_data, params={"id": f"eq.{article_id}"})


def insert_fallback_contexts(article_id: str, article_title: str) -> None:
    for role in REQUIRED_ROLES:
        row = {
            "article_id": article_id,
            "role": role,
            "headline": f"{role.capitalize()} lens: {article_title[:62]}",
            "why_it_matters": f"For a {role}, this update changes near-term decision making and risk-reward choices.",
            "role_analogy": f"Think of this like a weekly planning decision for a {role} in India.",
            "action_cards": _default_action_cards(role, article_title),
            "relevance_score": 0.68,
            "what_to_watch": "Track the next policy/earnings update window in 2-6 weeks.",
        }
        _ = supabase_request("POST", "article_contexts", data=row)


def seed_story_arcs_from_articles(articles_by_id: dict[str, dict[str, Any]]) -> None:
    # Build two timeline-rich arcs (3+ events each) from seeded article corpus.
    arc_specs = [
        {
            "topic_key": "rbi-rate-cycle-2026",
            "display_name": "RBI Rate Cycle 2026",
            "category": "Markets",
            "titles": [
                "RBI cuts repo rate by 25 bps to 6.25%, signals further easing",
                "Sensex crosses 82,000 for first time; FII inflows at 3-month high",
                "India's forex reserves touch $628 billion — highest since April 2024",
            ],
            "key_players": [
                {"name": "RBI", "role": "Monetary Regulator", "relevance": 0.98},
                {"name": "Indian Banks", "role": "Credit Transmission", "relevance": 0.89},
                {"name": "Retail Borrowers", "role": "Demand-side Impact", "relevance": 0.81},
            ],
        },
        {
            "topic_key": "policy-shifts-2026",
            "display_name": "Policy Shifts 2026",
            "category": "Policy",
            "titles": [
                "Budget 2026: Capex rises 22% to ₹11.2 lakh crore; infra push continues",
                "SEBI tightens F&O norms: Minimum lot size doubles, weekly expiries cut to one",
                "GST Council reduces EV tax to 5% from 12%, two-wheelers now competitive",
            ],
            "key_players": [
                {"name": "Finance Ministry", "role": "Fiscal Policy", "relevance": 0.95},
                {"name": "SEBI", "role": "Market Regulation", "relevance": 0.9},
                {"name": "GST Council", "role": "Tax Policy", "relevance": 0.86},
            ],
        },
    ]
    ...
    for spec in arc_specs:
        ...
        for title in spec["titles"]:
            article = next((a for a in articles_by_id.values() if a.get("title") == title), None)
            if not article:
                continue
            timeline.append({
                "date": article.get("published_at") or "2026-03-01T00:00:00Z",
                "event_title": article.get("title", "Update"),
                "event_summary": (article.get("eli5") or article.get("content") or "")[:220],
        ...
        existing = supabase_request(
            "GET",
            "story_arcs",
            params={"select": "id", "topic_key": f"eq.{spec['topic_key']}", "limit": "1"},
        )
        ...
        if existing and isinstance(existing, list) and len(existing) > 0:
            _ = supabase_request("PATCH", "story_arcs", data=payload, params={"id": f"eq.{existing[0]['id']}"})
        else:
            _ = supabase_request("POST", "story_arcs", data=payload)
...
async def seed_fresh():
    ...
    success, failed = 0, 0
    processed_articles: dict[str, dict[str, Any]] = {}
    ...
    for i, article_data in enumerate(DEMO_ARTICLES):
        ...
        existing = supabase_request("GET", "articles", params=params)
        ...
        if existing and isinstance(existing, list) and len(existing) > 0:
            ...
            # Insert new article
            insert_data = build_seed_insert_data(article_data)
            # Add resolution to force return=representation
            result = supabase_request("POST", "articles", data=insert_data, params={"select": "*"})
        ...
        if article and article.get("id"):
            ...
            processed_articles[article["id"]] = article
        ...
        try:
            ...
            if synthesis_result:
                ...
                if article and article.get("id"):
                    apply_fallback_article_fields(article["id"], article_data)
                    insert_fallback_contexts(article["id"], article_data["title"])
                failed += 1
    ...
    # Refresh processed article rows so timeline summaries can use synthesized values.
    for article_id in list(processed_articles.keys()):
        latest = supabase_request("GET", "articles", params={"select": "*", "id": f"eq.{article_id}", "limit": "1"})
        if latest and isinstance(latest, list):
            processed_articles[article_id] = latest[0]

    seed_story_arcs_from_articles(processed_articles)
...
def verify_seed():
    articles = supabase_request(
        ...
        params={
            "select": (
                "id,title,category,source,credibility_score,story_momentum,"
                "bharat_india_split,conflict_index,role_headlines,key_concepts,"
                "entities,sentiment,published_at,eli5,synthesis_briefing"
            )
        },
    )
    if not articles:
    ...
    required_article_fields = [
        "eli5", "synthesis_briefing", "category", "source", "credibility_score",
        "story_momentum", "bharat_india_split", "conflict_index", "role_headlines",
        "key_concepts", "entities", "sentiment", "published_at",
    ]

    for article in articles:
        empty_fields = [k for k in required_article_fields if article.get(k) in (None, "")]
        if empty_fields:
            missing_required_fields += 1
            print(f"  ⚠ ARTICLE FIELDS MISSING: {article['title'][:60]}")
            print(f"    Missing: {', '.join(empty_fields)}")

        contexts = supabase_request(
            "GET",
            "article_contexts",
            params={
                "select": "role,headline,why_it_matters,role_analogy,action_cards,relevance_score,what_to_watch",
        ...
        if contexts:
            for ctx in contexts:
                missing_ctx = [
                    k for k in ("headline", "why_it_matters", "role_analogy", "action_cards", "relevance_score", "what_to_watch")
                    if ctx.get(k) in (None, "")
                ]
                if missing_ctx:
                    incomplete_context_rows += 1
                    print(f"  ⚠ CONTEXT INCOMPLETE ({ctx.get('role')}): {article['title'][:52]}")
                    print(f"    Missing: {', '.join(missing_ctx)}")

    arcs = supabase_request("GET", "story_arcs", params={"select": "topic_key,timeline", "limit": "50"}) or []
    valid_arcs = 0
    for arc in arcs:
    ...
    print(f"Article rows missing required fields: {missing_required_fields}")
    print(f"Context rows missing required fields: {incomplete_context_rows}")
    print(f"Story arcs with >=3 timeline events: {valid_arcs}")
